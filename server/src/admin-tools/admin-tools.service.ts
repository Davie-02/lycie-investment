  ...
  async systemStatus() {
    ...
    return {
      ...
      services: [
        ...
        row("Lycie AI (Gemini)", this.gemini.hasGemini, this.gemini.hasGemini ? "Connected" : "Set GEMINI_API_KEY — needed for the deals finder, briefings and reading pictures."),
        row("Backup AI (Groq)", on(process.env.GROQ_API_KEY), on(process.env.GROQ_API_KEY) ? "Connected — answers when Gemini is busy or out of quota" : "Optional: GROQ_API_KEY (free at console.groq.com) keeps Lycie answering when Gemini's free quota runs out."),
