...
├── config/               siteConfig.ts — contact info, hours, About copy
...
Vehicle and hire-vehicle listings now come from the database via
`server/prisma/seed.ts`, not from a local frontend file. To change what's
displayed, edit the seed script and re-run `npm run prisma:seed`, or write
directly to the database — no frontend code needs to change either way, since
components read through `src/services/vehicles.service.ts`.

## Admin dashboard

Visit `/admin` (e.g. `http://localhost:5173/admin`) to manage vehicle and
hire-vehicle listings, manage other admin accounts (Owner/Manager/Viewer
roles), and view submitted form requests — without touching the database
directly. See `server/README.md` for how to bootstrap your first login —
nothing works until `ADMIN_EMAIL`, `ADMIN_PASSWORD_HASH`, and `JWT_SECRET`
are set in `server/.env` and `npm run prisma:seed` has been run.
